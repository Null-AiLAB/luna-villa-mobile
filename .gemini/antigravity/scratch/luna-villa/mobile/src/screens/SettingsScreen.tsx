...
import { api } from '../api';
import { useDebugStore, getLunaTime } from '../utils/debugStore';
...
export default function SettingsScreen({ onLogout, navigation }: Props) {
    ...
    const getGreeting = () => {
        const hour = getLunaTime().getHours();
    ...
    const loadStats = async () => {
        try {
            ...
            if (data) {
                if (debug.isEnabled && debug.mockAffinity !== null) {
                    data.affinity.level = debug.mockAffinity;
                    // 簡易的にランクも調整（必要なら）
                    if (debug.mockAffinity > 50) data.affinity.rank = '最愛のパートナー';
                    else if (debug.mockAffinity > 20) data.affinity.rank = '大切な存在';
                }
                setStats(data);
            }
        } catch { }
        ...
        <View style={[styles.container, { backgroundColor: theme.background || '#0D0B1A' }]}>
            ...
            <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
                ...
                >
                    ...
                    <Text style={[styles.debugButtonText, { color: theme.textMuted }]}>デバッグメニュー</Text>
                </TouchableOpacity>
